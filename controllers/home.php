...
while ($p = $posts->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($ip == 0) {
        $size = 180;
        $text = '<p>' . truncate(strip_tags($p['post_content']), 300, '...', true) . '<br><a href="/blog/' . $p['post_url'] . '" class="btn btn-secondary-outline btn-sm">Lire la suite <i class="fa-solid fa-chevron-right"></i></a></p>';
    }
